...
router.get("/eva/all", async (_req, res: Response) => {
  try {
    const evaluations = await Evatest.find().populate("imageId", "id contentType");
...
router.get("/eva/:id", async (req: Request, res: Response) => {
  try {
    const evaluation = await Evatest.findById(req.params.id).populate("imageId", "id contentType");
